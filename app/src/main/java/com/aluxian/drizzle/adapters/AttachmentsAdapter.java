...
public class AttachmentsAdapter extends MultiTypeInfiniteAdapter<Attachment> {
    ...
    protected void onAddItemTypes() {
        ...
        addItemType(new MultiTypeItemType<>(AttachmentItem.class, AttachmentItem.ViewHolder.class, R.layout.item_attachment));
    }

    @Override
    protected List<MultiTypeBaseItem<? extends MultiTypeBaseItem.ViewHolder>> mapLoadedItems(List<Attachment> items) {
        return Mapper.map(items, AttachmentItem::new);
    }

    public static class AttachmentItem extends MultiTypeBaseItem<AttachmentItem.ViewHolder> implements View.OnClickListener {
